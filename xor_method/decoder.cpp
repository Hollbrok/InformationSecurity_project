...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        fprintf(stderr, "Usage: <decoder> <input_file> <output_file> <key>\n");
        return EXIT_FAILURE;
    }

    std::string inputFileName = std::string(argv[1]);
    std::string decryptedFileName = std::string(argv[2]);
    std::string key = std::string(argv[3]);

    // Дешифрование файла
    xorEncryptDecrypt(inputFileName, decryptedFileName, key);
